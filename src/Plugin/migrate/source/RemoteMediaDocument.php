...
/**
 * Source plugin for remote media documents.
 *
 * @MigrateSource(
 *   id = "remote_media_document"
 * )
 */
class RemoteMediaDocument extends SqlBase {
  ...
  public function query() {
    ...
    $query->innerJoin('media__field_media_document', 'fmd_doc', 'fmd_doc.entity_id = mfd.mid');
    $query->innerJoin('file_managed', 'fmd', 'fmd.fid = fmd_doc.field_media_document_target_id');
    $query->addField('fmd', 'uri', 'file_uri');
    $query->condition('mfd.bundle', 'document');
